<template>
  <div>
    <a-button v-if="!srcProp" type="primary" @click="selectImage">
      <template #icon>
        <UploadOutlined />
      </template>
      <span>Tải lên</span>
    </a-button>
    <input ref="imagePickerRef" type="file" accept="image/*" hidden @change="pickImage" />

    <div v-if="srcProp" class="relative" :style="{ width: `${width}px`, height: `${height}px` }">
      <a-image
        :src="srcProp"
...
const pickImage = () => {
  ...
  if (files && files.length !== 0) {
    ...
    reader.onload = function (e) {
      emits('change', { src: e.target.result, file })
    }
...
export default defineComponent({
  components: { UploadOutlined, CloseCircleFilled }
})
</script>
